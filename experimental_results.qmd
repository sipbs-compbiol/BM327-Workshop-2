...
These results were obtained using a [Bayesian multilevel regression](https://widdowquinn.github.io/Teaching-Stan-Hierarchical-Modelling/) statistical approach, and are presented graphically below. As the UPEC genome comprises a chromosome and an accompanying plasmid, these are shown separately.
...
We are interested in genes that are more abundant than expected after mouse passage, but not after control passage: The genes indicated in _green_ in @fig-chrom-treat and @fig-plasmid-treat.
...
In the mouse-passaged isolates, four regions known as "S-loops" were seen to be enriched. These encode for proteins annotated as being involved with urea degradation, effector proteins associated with adherence to bovine tissue, a surfactin, and hæm utilisation. The known functions of these genes are consistent with a role in adherence to tissue, and infection.

### Plasmid-borne genes

A comparison of @fig-plasmid-control and @fig-plasmid-treat shows that no plasmid-borne genes are overrepresented in the control arm, but that a run of several genes corresponding to an operon for a Type II Secretion System (T2SS) are highlighted in the mouse arm. This T2SS has been reported to play a role in adherence to mammalian host tissues, and in biofilm formation. It is considered an important virulence factor. 
